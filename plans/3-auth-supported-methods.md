...
需要明確定義 App 支援哪些登入方式，並把既有的密碼登入 / 匿名登入收斂到這個政策。
...
1. 重構 `auth_service.dart`（移除密碼方法，改 Email Link）。
2. 重構登入 UI `signed_out_view.dart` 與 l10n。
3. 實作 Email Link 的 deep link reception（程式面）。
4. 設定 Android App Links（網域 `seek-player-f724e.web.app`）。
5. 加入 Facebook 登入（程式面，UI 暫隱藏）。
6. Firebase / Meta 後台設定與部署驗證（**待辦**）。

## 各步驟說明

### 1. 重構 auth_service.dart ✅
移除密碼方法，新增 `sendSignInLink` / `isSignInLink` / `signInWithEmailLink`
（使用 `ActionCodeSettings`，android `com.js.seek_player`、iOS `com.js.seekPlayer`）。
...
### 3. Deep link reception（程式面）✅
新增 `email_link_controller.dart`
...
- `public/.well-known/assetlinks.json`：package `com.js.seek_player`，
  **已填入 upload/release SHA-256**（`72:98:0A:...:28:20`）。
  ...
  [docs/android_keystore_sha256.md](../docs/android_keystore_sha256.md) 步驟 3(c)）。

### 5. Facebook 登入（2026-06-12，UI 暫隱藏）✅
- `auth_service.dart` 新增 `signInWithFacebook()`
  （`signInWithProvider(FacebookAuthProvider())`；`web-context-canceled` / `canceled`
  視為使用者取消、靜默返回）。
- `signed_out_view.dart` 在 Google 下方加「使用 Facebook 登入」按鈕；
  l10n 新增 `account_sign_in_facebook`（en / zh_TW / zh_CN）。
- **決策：不採用 `flutter_facebook_auth` 套件**（曾加入後移除），避免引入
  Facebook SDK 與 Android `strings.xml` / iOS `Info.plist` 原生設定；
  代價是登入走瀏覽器而非原生 FB App 切換。
